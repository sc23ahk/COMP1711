    ...
    while (1)
    {
        ...
        case 'c':
            ...
            while (fgets(line, buffer_size, input)){
                ...
                // using the & operator to pass in a pointer to the bloodIron so it stores it
                tokeniseRecord(line, ",", daily_readings[counter].date, &daily_readings[counter].bloodIron);
                if(daily_readings[counter].bloodIron < low){
                    low = daily_readings[counter].bloodIron;
                }
                counter++;
            }
            printf("Your lowest lood iron was %.2f\n", low);
            break;
        ...
        case 'd':
            ...
            while (fgets(line, buffer_size, input)){
                ...
                // using the & operator to pass in a pointer to the bloodIron so it stores it
                tokeniseRecord(line, ",", daily_readings[counter].date, &daily_readings[counter].bloodIron);
                if(daily_readings[counter].bloodIron > high){
                    high = daily_readings[counter].bloodIron;
                }
                counter++;
            }
            printf("Your highest lood iron was %.2f\n", high);
            break;
        ...
        case 'e':
            ...
            char month[4];
            char monthW[4];
            printf("What month do you want to look at?\n");
            scanf("%s", monthW);
            while (fgets(line, buffer_size, input)){
                // split up the line and store it in the right place
                // using the & operator to pass in a pointer to the bloodIron so it stores it
                tokeniseRecord(line, ",", daily_readings[counter].date, &daily_readings[counter].bloodIron);
                for (int i=3; i < 6; i++){
                    month[i] = daily_readings[counter].date[i];
                }
                printf("%s\n", month);
                /*
                if (month == monthW){
                    printf("%f", daily_readings[counter].bloodIron);
                }
                */
                counter++;
            }
